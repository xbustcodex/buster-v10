...
class RealPerceptionLoop:
    \"\"\"
    RealPerceptionLoop is the practical bridge between perception inputs and
    Buster's World Model / Companion systems.

    It is intentionally lightweight and safe:
    - works without camera/microphone dependencies
    - accepts synthetic observations for tests and integration
    - can be started/stopped by runtime code
    - always returns a stable contract for callers
    \"\"\"
    ...
    def observe(
        ...
        **kwargs: Any,
        ...
        # Backwards/forwards compatibility with possible caller names.
    ...
    def _maybe_generate_speech(
        ...
        # Stable contract: key is always present. Value is None for low value events.
...
def main() -> None:
    print("=== Applying RealPerceptionLoop Final Contract Repair ===")
    TARGET.parent.mkdir(parents=True, exist_ok=True)
    TARGET.write_text(CODE.strip() + "\n", encoding="utf-8")
    print(f"[WRITE] {TARGET.relative_to(ROOT)}")
    print("\nSUCCESS: RealPerceptionLoop speech/start contract repaired.")
