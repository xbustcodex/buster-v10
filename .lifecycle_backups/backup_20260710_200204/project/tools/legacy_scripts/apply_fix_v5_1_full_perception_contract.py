...
PLANNER = ROOT / "buster" / "brain" / "planner" / "perception_planner_bridge.py"
DASH = ROOT / "buster" / "workspace" / "perception_dashboard.py"
WIDGET = ROOT / "buster" / "ui" / "widgets" / "perception_loop_widget.py"

PLANNER_CODE = '''from __future__ import annotations
...
DASH_CODE = '''from __future__ import annotations
...
WIDGET_CODE = '''from __future__ import annotations
...
def main() -> None:
    print("=== Applying v5.1 Full Perception Test Contract Repair ===")
    PLANNER.parent.mkdir(parents=True, exist_ok=True)
    DASH.parent.mkdir(parents=True, exist_ok=True)
    WIDGET.parent.mkdir(parents=True, exist_ok=True)
    PLANNER.write_text(PLANNER_CODE, encoding="utf-8")
    DASH.write_text(DASH_CODE, encoding="utf-8")
    WIDGET.write_text(WIDGET_CODE, encoding="utf-8")
    print(f"[WRITE] {PLANNER.relative_to(ROOT)}")
    print(f"[WRITE] {DASH.relative_to(ROOT)}")
    print(f"[WRITE] {WIDGET.relative_to(ROOT)}")
    print("\\nSUCCESS: Full v5.1 perception contract repaired.")
