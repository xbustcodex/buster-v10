...
TARGET = ROOT / "buster" / "repository" / "semantic_graph.py"
...
class SemanticRepositoryGraph:
    ...
    def who_calls(self, name: str) -> list[CallEdge]:
        q = name.lower()
        return [c for c in self.calls if q in c.callee.lower()]

    def inherits_from(self, name: str) -> list[InheritanceEdge]:
        q = name.lower()
        return [i for i in self.inheritance if q in i.parent.lower()]
    ...
    def ask(self, question: str) -> str:
        ...
        if q.startswith("who calls "):
            target = q.replace("who calls ", "").strip(" ?")
            return self.format_calls(self.who_calls(target), f"Who calls {target}")

        if "inherit" in q:
            target = q.replace("which classes inherit", "").replace("inherits from", "").strip(" ?")
            return self.format_inheritance(self.inherits_from(target), f"Classes inheriting {target}")

        if "depend" in q or "import" in q:
            target = q.replace("which modules depend on", "").replace("depends on", "").replace("imports", "").strip(" ?")
...
test = ROOT / "test_v3_3_3_semantic_repository_graph.py"
write(test, r'''
from pathlib import Path
...
print()
print("Buster v3.3.3 Semantic Repository Graph patch complete.")
print("Run:")
print("python test_v3_3_3_semantic_repository_graph.py")